...
You've been tasked with providing a report that addresses a series of objectives (see below). Your report should consist of a explanatory text as well as table and figures, as well as the R code you create for your analysis. This could be submitted either as a Word document and a R script, or in a comprehensive [R Markdown](https://rmarkdown.rstudio.com/) file (this is the preferred method). 
...
### Bonus Objective: Logistic Regression

Scoring points is great, but it is also important to be efficient (there are only so many shots after all). Provide a similar prediction has in Objective 4, expect this time use `nba_fgp` as the response/dependent variable. Linear regression won't work for this binomial variable, so take a look the `glm()` function. 
